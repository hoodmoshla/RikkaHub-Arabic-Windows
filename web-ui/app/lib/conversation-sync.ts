 ...
 * 向前逐个比对内容戳,一致才保留进合并结果(保持对象身份),首个不一致即停——
 * 之后的更早历史被丢弃,用户向上滚动时经分片端点重拉新版。
 * 全量快照(nodesOffset 0/缺省)或本地无可合并状态时,原样采用新快照。
 */
export function mergeConversationSnapshot(
  ...
  if (inOffset === 0 || !existing || existing.id !== incoming.id) return incoming;
  const exOffset = existing.nodesOffset ?? 0;
  const exStamps = existing.nodeStamps;
  const inStamps = incoming.nodeStamps;
  if (!exStamps || !inStamps || exOffset >= inOffset) return incoming;
  ...
  if (keepStart === inOffset) return incoming;

  return {
    ...incoming,
    messages: [
      ...existing.messages.slice(keepStart - exOffset, inOffset - exOffset),
      ...incoming.messages,
    ],
    nodesOffset: keepStart,
  };
}
