import { JsonValue, Model, Provider, ProxyMode, ProxyConfig, Assistant, AsrProvider, TtsProvider, Message, MessageNode, WriteStrategy, Conversation, DailyStat, StoredFile, State, SearchService, GlobalMemoryFile, AssistantMemoryFile, GitHubSkillInfo, GitHubSkillFile, ApiMessage, FontWeightFile, FontEntry, ManifestEntry, BuiltinManifest, StreamHooks, AuxiliaryTextOptions } from "./foundation/types";
...
import { handleDataRoutes } from "./api/handlers/data";
...
async function routeApi(request: Request, url: URL) {
  ...
  { const r = await handleDataRoutes(request, url, path); if (r) return r; }
  ...
  { const r = await handleUpdateRoutes(request, url, path); if (r) return r; }
