...
import { handleMediaRoutes } from "./api/handlers/media";
...
import { broadcastConversation, broadcastList, broadcastMemoryUpdate, broadcastNodeUpdate, broadcastSettings, conversationClients, listClients, openSse, scheduleNodeBroadcast } from "./api/sse";
import {
  applyCustomBody,
  applyRequestHeaders,
  builtinProviderRank,
...
async function routeApi(request: Request, url: URL) {
  ...
  { const r = await handleSettingsRoutes(request, url, path); if (r) return r; }
  ...
  { const r = await handleMediaRoutes(request, url, path); if (r) return r; }
